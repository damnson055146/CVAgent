...
# 使用 app.database 中的异步 get_db 依赖
...
async def delete_document(
    ...
    # 查找文档
    document_to_delete = await db.get(document_models.Document, doc_id)
    ...
    # 验证用户权限
    if document_to_delete.user_id != current_user.id:
    ...
    # 软删除所有相关的版本
    versions_result = await db.execute(select(document_models.DocumentVersion).where(
        document_models.DocumentVersion.document_id == doc_id,
        document_models.DocumentVersion.deleted_at.is_(None)
    ))
    versions_to_delete = list(versions_result.scalars().all())
...
async def get_user_documents_by_type(
    ...
    try:
        ...
        result = await db.execute(
            select(document_models.Document)
            .where(
...
async def save_document_endpoint(
    ...
    """
    try:
        # 覆盖请求体中的 user_id，统一使用鉴权用户
        payload.user_id = current_user.id

        # 1) 查找并校验文档
        doc = await db.get(document_models.Document, doc_id)
        if not doc:
        ...
        cnt_res = await db.execute(
            select(func.count()).select_from(document_models.DocumentVersion).where(
                document_models.DocumentVersion.document_id == doc.id,
                document_models.DocumentVersion.deleted_at.is_(None)
            )
        )
        next_ver = int(cnt_res.scalar() or 0) + 1
...
async def get_document_history(
    ...
    try:
        ...
        result = await db.execute(
            select(document_models.DocumentVersion)
            .where(
                document_models.DocumentVersion.document_id == doc_id,
                document_models.DocumentVersion.deleted_at.is_(None)
            )
...
async def get_version_content(
    ...
    """
    # 查找版本并验证用户权限
    version = await db.get(document_models.DocumentVersion, version_id)
    ...
    # 检查文档是否已被软删除
    parent_doc = await db.get(document_models.Document, version.document_id)
    if parent_doc.deleted_at is not None:
...
async def delete_version(
    ...
    # 查找版本
    version_to_delete = await db.get(document_models.DocumentVersion, version_id)
    ...
    parent_document = await db.get(document_models.Document, version_to_delete.document_id)
    if parent_document.user_id != current_user.id:
    ...
    if parent_document.current_version_id == version_to_delete.id:
        ...
        new_latest_version = res_latest.scalars().first()
...
# 为版本保存名称
@app.post(
    "/api/versions/{doc_id}/version-name-save",
    status_code=200,
    tags=["版本 (Versions)"]
)
async def save_user_profile_to_version(
        doc_id: uuid.UUID = Path(..., description="目标版本ID (document_versions.id)"),
        payload: dict = Body(..., example={"user_profile": "xxx"}),
        db: AsyncSession = Depends(get_db),
        current_user: user_models.User = Depends(get_current_user_flexible)
):
    """为版本保存/更新名称（使用 version.user_profile 字段承载）。"""
    from sqlalchemy.orm import joinedload
    # 查找版本并关联父文档
    version = await db.get(document_models.DocumentVersion, doc_id)
    ...
    # 校验用户
    parent_document = await db.get(document_models.Document, version.document_id)
    if str(parent_document.user_id) != str(current_user.id):
...
async def list_personal_statement_profiles(
    payload: dict = Body(..., example={"name": "张三"}),
    db: AsyncSession = Depends(get_db),
    current_user: user_models.User = Depends(get_current_user_flexible)
):
    # 校验 name
    ...
    # 由 access_token 鉴权注入的当前用户ID
    uid = current_user.id
    rows = await db.execute(text(
        """
        SELECT id, name, profile_md, created_at
        FROM personal_statement_profiles
        WHERE user_id = :user_id AND name = :name
        ORDER BY created_at DESC
        """
    ), {"user_id": str(uid), "name": name_value})
